...
class S6(nn.Module):
    def __init__(self, seq_len, d_model, state_size, device):
        ...
        self.A = nn.Parameter(F.normalize(torch.ones(
            d_model, state_size, device=device), p=2, dim=-1))
        nn.init.xavier_uniform_(self.A)

        self.B = torch.zeros(params.batch_size, self.seq_len,
                             self.state_size, device=device)
        self.C = torch.zeros(params.batch_size, self.seq_len,
                             self.state_size, device=device)

        self.delta = torch.zeros(
            params.batch_size, self.seq_len, self.d_model, device=device)
        self.dA = torch.zeros(params.batch_size, self.seq_len,
                              self.d_model, self.state_size, device=device)
        self.dB = torch.zeros(params.batch_size, self.seq_len,
                              self.d_model, self.state_size, device=device)

        # h should have dimensions [batch_size, seq_len, d_model, state_size]
        self.h = torch.zeros(params.batch_size, self.seq_len,
                             self.d_model, self.state_size, device=device)
        self.y = torch.zeros(params.batch_size, self.seq_len,
    ...
    def forward(self, x):
        ...
        h = torch.einsum('bldn,bldn->bldn', self.dA, h) + \
...
class MambaBlock(nn.Module):
    def __init__(self, seq_len, d_model, state_size, device):
        ...
        self.conv = nn.Conv1d(
            seq_len, seq_len, kernel_size=3, padding=1, device=device)
    ...
    def forward(self, x):
        ...
        # Swish activation can be implemented as x * sigmoid(x)
        x_act = F.silu(x_ssm)
