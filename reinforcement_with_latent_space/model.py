...
class PlanRecognition(nn.Module):
    ...
    def forward(self, video_embeded, proprioceptions):

        x = torch.cat([video_embeded, proprioceptions], dim=-1)
        # LSTM Layers
        x, _ = self.lstm1(x)
        x, _ = self.lstm2(x)
        # Latent variable
...
class PlanRecognitionTransformer(nn.Module):
    def __init__(self):
        ...
        self.epsilon = 1e-4
        self.max_seq_length = 1000   # Maximum sequence length of a demonstration
        self.in_dim = params.vision_embedding_dim + params.proprioception_dim
        self.latent_dim = params.latent_dim
        self.bath_size = params.batch_size

        self.position_embedding = nn.Embedding(
            self.max_seq_length, self.in_dim)

        # Define the Transformer encoder layer (self-attention on the same sensor embedding sequence)
        encoder_layers = nn.TransformerEncoderLayer(
            d_model=self.in_dim, nhead=self.nhead, dim_feedforward=self.layer_size, batch_first=True)
    ...
    def forward(self, video_embeded, proprioceptions):
        ...
        positions = torch.arange(seq_length, device=x.device).expand(
            (self.bath_size, seq_length)).contiguous()
...
class LogisticActor(nn.Module):
    def __init__(self, layer_size=1024, epsilon=1e-4):
        ...
        self.in_dim = 2*params.vision_embedding_dim + \
            params.latent_dim + params.proprioception_dim
    ...
    def forward(self, vision_embeded, proprioception, latent, goal_embeded):
        ...
        x = torch.cat([vision_embeded, proprioception,
                      latent, goal_embeded], dim=-1)
...
class DirectActorTransformer(nn.Module):
    def __init__(
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.in_dim, self.action_dim), nn.Tanh())
    ...
    def forward(self, vision_embeded, proprioception, latent, goal_embeded):
        ...
        positions = torch.arange(self.current_length, device=self.device).expand(
            (self.batch_size, self.current_length)).contiguous()
        pos_embeddings = self.position_embedding(positions)
        # Apply position embeddings to the valid portion of the buffer
        # x[:, :self.current_length, :] += pos_embeddings

        x = torch.cat([vision_embeded, proprioception, latent, goal_embeded], dim=-1)
        # x = x.unsqueeze(1)
        # x = x.repeat(1, self.max_seq_length, 1)
    ...
    def get_action(self, vision_embeded, proprioception, latent, goal_embeded, attention_mask=None):
        """
        Get the action for the current state
        """
        vision_embeded = vision_embeded[:, -self.max_seq_length:, :]
        proprioception = proprioception[:, -self.max_seq_length:, :]
        latent = latent[:, -self.max_seq_length:, :]
        goal_embeded = goal_embeded[:, -self.max_seq_length:, :]

        # pad all tokens to sequence length
        attention_mask = torch.cat([torch.zeros(
            self.max_seq_length-vision_embeded.shape[1]), torch.ones(vision_embeded.shape[1])])
        attention_mask = attention_mask.to(
            dtype=torch.long, device=self.device).reshape(1, -1)

        vision_embeded = torch.cat([torch.zeros(vision_embeded.shape[0], self.max_seq_length-vision_embeded.shape[1],
                                                self.vision_embedding_dim), vision_embeded], dim=1)
        proprioception = torch.cat([torch.zeros(proprioception.shape[0], self.max_seq_length - proprioception.shape[1], self.proprioception_dim), proprioception],
                                   dim=1)
        latent = torch.cat([torch.zeros(
            latent.shape[0], self.max_seq_length - latent.shape[1], self.latent_dim), latent], dim=1)
        goal_embeded = torch.cat([torch.zeros(goal_embeded.shape[0], self.max_seq_length -
                                 goal_embeded.shape[1], self.vision_embedding_dim), goal_embeded], dim=1)
        # timesteps = torch.cat([torch.zeros(timesteps.shape[0], self.max_seq_length - timesteps.shape[1]), timesteps], dim=1)

        if attention_mask is None:
            # attention mask: 1 if can be attended to, 0 if not
            attention_mask = torch.ones((self.batch_size, self.max_seq_length))


        action = self.forward(
            vision_embeded, proprioception, latent, goal_embeded)
        return action
...
class Critic(nn.Module):
    def __init__(self, layer_size=1024):
        ...
        self.in_features = params.vision_embedding_dim + \
            params.proprioception_dim + params.action_dim
